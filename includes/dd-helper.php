...
function getDashCounter($id, $raw = false)
{
  ...
  $jstimeout = ddGetSchedulerNumeric($Counter->timeperiod);
	
	$Numbers = str_split(number_format($Counter->value));
  
    ?>    
    <div class="dd_counter" data-counterid="<?php echo 'dd_counter_'.$id; ?>" data-value="<?php echo $Counter->value;?>" data-incvalue="<?php echo $Counter->inc_range_start; ?>" data-time="<?php echo $jstimeout; ?>">
...
function ddGetSchedulerNumeric($Interval)
{
    $TimeNumeric = 0;
    ...
    switch ($Interval) {
      ...
      case 'everytwelvehours':
        $TimeNumeric = 12 * HOUR_IN_SECONDS;
        break;

      case 'everyday':
        $TimeNumeric = DAY_IN_SECONDS;
        break;

      case 'everythreedays':
        $TimeNumeric = 3 * DAY_IN_SECONDS;
        break;

      case 'everyweek':
        $TimeNumeric = 7 * DAY_IN_SECONDS;
        break;        
      
      case 'everymonth':
        $TimeNumeric = 30 * DAY_IN_SECONDS;
        break; 
